...
export const downloadAsExcel = (entries: ProcessedNameEntry[]) => {
  ...
  XLSX.utils.book_append_sheet(workbook, worksheet, 'Verified Names');
  
  XLSX.writeFile(workbook, 'NameVerifier_Results.xlsx');
};
...
export const downloadAsHtml = (entries: ProcessedNameEntry[], sources: GroundingSource[]) => {
  const htmlContent = `
    ...
    <head>
      ...
      <style>
        body { font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif; line-height: 1.6; color: #333; max-width: 1200px; mx-auto; padding: 20px; }
        ...
        .verified { color: #16a34a; font-weight: 500; }
        .unverified { color: #d97706; font-weight: 500; }
